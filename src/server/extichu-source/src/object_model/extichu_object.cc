...
function<bool(const Ptr<User> &)> User::MatchByUserID(const string &userid, MatchCondition cond) {
  function<string(const Ptr<User> &)> attribute_getter = bind(&User::GetUserID, _1);
  return bind(&CompareAttribute<User, string>, _1, userid, attribute_getter, cond);
}


function<bool(const Ptr<User> &)> User::MatchByNickname(const string &nickname, MatchCondition cond) {
  function<string(const Ptr<User> &)> attribute_getter = bind(&User::GetNickname, _1);
  return bind(&CompareAttribute<User, string>, _1, nickname, attribute_getter, cond);
};


void User::RegisterUserIDTrigger(const TriggerCondition &condition, const TriggerAction &action) {
  Object::RegisterAttributeTrigger("User", "UserID", condition, action);
}


void User::RegisterNicknameTrigger(const TriggerCondition &condition, const TriggerAction &action) {
  Object::RegisterAttributeTrigger("User", "Nickname", condition, action);
}


void User::SelectByUserID(const Object::ConditionType &cond_type, const string &cond_value, const Object::SelectCallback &callback) {
  Object::Select("User", "UserID", "", cond_type, AttributeValue(cond_value), callback);
}


void User::SelectByNickname(const Object::ConditionType &cond_type, const string &cond_value, const Object::SelectCallback &callback) {
  Object::Select("User", "Nickname", "", cond_type, AttributeValue(cond_value), callback);
}
...
Ptr<User::OpaqueData> User::CreateOpaqueDataFromJson(const Json &json) {
  ...
  if (json.HasAttribute("Nickname")) {
    if (not json["Nickname"].IsString()) {
      LOG(ERROR) << "wrong 'Nickname' value: type mismatch";
      return Ptr<OpaqueData>();
    }
    data->nickname = json["Nickname"].GetString();
  }
  return data;
}


Ptr<User> User::Create(const string &userid) {
  AttributeValueMap key_params;
  key_params["UserID"].reset(new AttributeValue(userid));

  const Ptr<const ObjectModel> &model(ObjectModel::FindModel("User"));
  BOOST_ASSERT(model);

  // try to fetch by same key values.
  if (FetchByUserID(userid, kWriteLock)) {
...
void User::Fetch(
  ...
  if (result) {
    ...
    Object::Fetch(model, ids, lock_type, NULL);
  }
}


Ptr<User> User::FetchByUserID(const string &value, LockType lock_type) {
  Ptr<AttributeValue> key_value(new AttributeValue(value));
  const Ptr<const ObjectModel> &model(ObjectModel::FindModel("User"));
  BOOST_ASSERT(model);

  Ptr<Object> obj = Object::Fetch(model, "UserID",  key_value, lock_type);
  if (not obj)
    return kNullPtr;
...
void User::FetchByUserID(
  ...
  Object::Fetch(model, "UserID", key_values, lock_type, &objs);
...
bool User::PopulateFrom(const Ptr<OpaqueData> &opaque_data) {
  SetNickname(opaque_data->nickname);
  return true;
}
...
void User::SetUserID(const string &value) {
  BOOST_ASSERT(object());object()->SetString("UserID", value);}
...
void User::SetNickname(const string &value) {
  BOOST_ASSERT(object());object()->SetString("Nickname", value);}
...
void RegisterUserModel() {
  ...
  attrs.push_back(Ptr<const AttributeModel>(
      new AttributeModel(
          "UserID", fun::AttributeModel::kString, 0, true, false, false, false, false)));
  attrs.push_back(Ptr<const AttributeModel>(
      new AttributeModel(
          "Nickname", fun::AttributeModel::kString, 0, false, false, false, false, false)));
...
void DumpJson(const User &obj, Json *dest) {
  ...
  Json &out = *dest;
  out["UserID"].SetString(obj.GetUserID());
  out["Nickname"].SetString(obj.GetNickname());
}
...
CsApiHandler::CsApiHandler() : schemas_(boost::assign::map_list_of
    ("User", "{\"User\": {\"propertylist\": [{\"readonly\": true, \"type\": \"string\", \"name\": \"UserID\", \"key\": true}, {\"type\": \"string\", \"name\": \"Nickname\"}], \"type\": \"object\"}}").convert_to_container<boost::unordered_map<string, string> >()),
...
bool FetchUser(const std::string &key, Json &out) {
  Ptr<User> obj = User::FetchByUserID(key);
